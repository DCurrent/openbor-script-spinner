...
int dc_spinner_get_instance()
{
	...
	// Get instance value.
	result = getlocalvar(DC_SPINNER_MEMBER_INSTANCE);
...
void dc_spinner_set_instance(int value)
{
	...
	if (value != DC_SPINNER_DEFAULT_INSTANCE)
	{
		setlocalvar(DC_SPINNER_MEMBER_INSTANCE, value);
	}
}
...
void dc_spinner_reset_instance()
{
	int instance;
	...
	dc_spinner_free_instance();

	// If the instance ID isn't default, then restore 
	// it to memory. Either way we now have a clean
	// copy of the current instance.
	if (instance != DC_SPINNER_DEFAULT_INSTANCE)
	{
		dc_spinner_set_instance(instance);
	}
}
...
void dc_spinner_free_instance()
{
	...
	for (i = 0; i < DC_SPINNER_MEMBER_THE_END; i++)
	{
		// Concatenate the variable ID.
		id = instance + DC_SPINNER_BASE_ID + i;
...
// to import.
void dc_spinner_export_instance()
{
	int instance;
	int i;
	char id;
...
void dc_spinner_import_instance()
{
	int instance;
	int i;
	char id;
...
void dc_spinner_free_export_instance()
{
	int instance;
	int i;
	char id;
